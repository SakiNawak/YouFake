		...
		function(data)
		{
			...
			else
			{
				...
				if(data.msg)
				{
					...
					_cb.throwHeadMsg('success', data.msg+"!", 5000,true);
				}
				if(data.err)
					alert(data.err);
			}
		...
		function(data)
		{
			...
			else
			{
				if(data.msg)
				{
					...
					_cb.throwHeadMsg('success', data.msg, 3000,true);
				}
				if(data.err)
				{
					_cb.throwHeadMsg('danger', data.err, 3000,true);
				}
			}
		},'json');
	}
...
function decode64(input) {
		...
		$.post(page,formObjectData,
			function(data)
			{
				...
				else
				{
					...
					if(data.err!='')
					{
						_cb.throwHeadMsg('danger', data.err, 5000,true);
						var str = data.err;
	...
	function add_beat_comment_js(form_id,type,beat_id)
	{   
		$("#add_comment_result").css("display","block");
		$("#add_comment_button_"+beat_id).val('Adding...');
		$("#add_comment_button_"+beat_id).attr("disabled",true);
		$(".add-reply").attr("disabled",true);
		...
		$.post(page,formObjectData,
			function(data)
			{
				...
				else
				{
					...
					{
						_cb.throwHeadMsg('danger', data.err, 5000,true);
						$("#add_comment_button_"+beat_id).attr("disabled",false);
					
					}
					else if(data.msg!='')
					{
						$('.comment_box_'+beat_id).val("");
						$('#Showcommnt_'+beat_id).collapse('show');
						$("#add_comment_button_"+beat_id).attr("disabled",false);

					}
				if(data.cid)
				{   
					get_beat_comment(data.cid,data.type_id,beat_id);
				}

			}
		},'json');
	}
	function get_beat_comment(id,type_id,beat_id)
	{
		$.post(page, 
		{ 	
			mode : 'get_beat_comment',
			cid : id,
			type_id : type_id
		},
		function(data)
		{
			...
			{	
				cData = data.comment;
				elem = document.getElementById("comment__list_"+beat_id);
				var z = document.createElement('li');
				/*
				// ES6 syntax translated below
				z.innerHTML = `
					<li class="commentsList__item" id="comment_${cData.comment_id}" {$comment_attr} style="">
                        <div class="commentItem">
                            <div class="commentItem__read">
                                <a href="#" class="commentItem__avatar">
                                    <img class="img-circle audio-thumb img-responsive" src="${cData.avatar_url}">
                                </a>
                                <div class="commentItem__content">
                                    <span class="commentItem__username">
                                        <a href="#">${cData.username}</a>
                                        <p class="commentItem__timestamp">${cData.date_added}</p>
                                    </span>
                                ...
                                <div class="commentItem__meta">
                                    <div class="commentItem__controls">
                                        ...
                                        <button class="btn btn-primary dropdown-toggle more_controls" type="button" data-toggle="dropdown">
                                            <i class="fa fa-ellipsis-v" aria-hidden="true"></i>
                                        </button>
                                        <ul class="dropdown-menu comment-drop pull-right">
                                                <li> 
                                                    <a href="javascript:void(0)" onclick="delete_comment('${cData.comment_id}','${cData.type}','${cData.type_id}')">Delete</a>
                                                </li>
                                        </ul>
                                    </div>
                                </div>
				...
				`;
				*/
				// tranpiled string litral
				z.innerHTML = "<li class=\"commentsList__item\" id=\"comment_" + cData.comment_id + "\" {$comment_attr} style=\"\"><div class=\"commentItem\"><div class=\"commentItem__read\"><a href=\"#\" class=\"commentItem__avatar\"><img class=\"img-circle audio-thumb img-responsive\" src=\"" + cData.avatar_url + "\"></a><div class=\"commentItem__content\">\n<span class=\"commentItem__username\"><a href=\"#\">" + cData.username + "</a><p class=\"commentItem__timestamp\">&nbsp;" + cData.date_added + "</p></span><div class=\"commentItem__body\"><span><p>" + cData.comment + "</p></span></div></div><div class=\"commentItem__meta\"><div class=\"commentItem__controls\"><button class=\"btn btn-primary dropdown-toggle more_controls\" type=\"button\" data-toggle=\"dropdown\"><i class=\"fa fa-ellipsis-v\" aria-hidden=\"true\"></i></button><ul class=\"dropdown-menu comment-drop pull-right\"><li><a href=\"javascript:void(0)\" onclick=\"delete_comment('" + cData.comment_id + "','" + cData.type + "','" + cData.type_id + "')\">Delete</a></li></ul></div></div></div></div></li>";
				elem.prepend(z);
			}
	...
	function reply_box(cid,type,type_id)
	{
		...
		html += '<textarea style="background-color:#d3d3d3;" name="comment" id="reply_box_'+cid+'" class="form-control" placeholder="Reply to '+replying_to_user+'..."></textarea>';
		html += '<i class="remove-'+cid+' remove-icon" onclick="remove_reply_box('+cid+')">';
		html += '<span style="color:#006dcc;cursor:pointer">';
		html += '<strong class="glyphicon glyphicon-chevron-up" style="color:#007fb3;margin-right:18px;margin-top:5px;" ></strong>';
		html += '</span>';
		html += '</i>';
		html += '</div>';
	...
	function edit_comment(cid,type,type_id)
	{	
		...
		$.ajax({
		    ...
		    success: function(response)
		    {
		        if(response)
		        {
			       ...
			       var html = '<form name="edit_box" method="post" id="edit_form_'+cid+'" onsubmit="return false;">';
					html += '<div class="textarea-comment clearfix">';
					html += '<textarea style="background-color:#d3d3d3;" name="comment" id="update_box_'+cid+'" class="form-control" placeholder="Add updated comment..." >'+response.comment+'</textarea>';
					html += '<i class="remove-'+cid+' remove-icon" onclick="remove_reply_box('+cid+')">';
					html += '<span style="color:#006dcc;cursor:pointer">';
					html += '<strong class="glyphicon glyphicon-chevron-up" style="color:#007fb3;margin-right:18px;margin-top:5px;" ></strong>';
					html += '</span>';
					html += '</i>';
					html += '</div>';
					html += '<input type="button" name="update_comment" id="update_comment_button_'+cid+'" class="btn btn-primary pull-right" onclick="update_comment_js(\'edit_form_'+cid+'\',\''+type+'\',\''+cid+'\',\''+type_id+'\')" value="Update">';
					html += '</form>';
					$('.reply-box-' + cid).html(html).slideDown("slow");
					$('#update_box_' + cid).focus();
	...
	function update_comment_js(form_id,type,cid,obj_id)
	{   
		
		$("#update_comment_button_"+cid).val('Updating...');
		$("#update_comment_button_"+cid).attr("disabled",true);
		var formObjectData = $('#' + form_id).serializeArray();
		var commentObj = formObjectData[0];
		...
		$.ajax({
		    ...
		    success: function(response)
		    {
		        if(response)
		        {
					...
					{	
						$('.reply-box-' + cid).slideUp("slow");
						_cb.throwHeadMsg('success', response.msg, 3000,true);
						update_comment_content(cid);
					}
					if(response.err)
					{
						$('.reply-box-' + cid).slideUp("slow");
						_cb.throwHeadMsg('danger', response.err, 3000,true);
					}
		        }
		    } 
	   	});
	...
	function update_comment_content (cid) {
		$.ajax({
		    type: "POST",
		    url: baseurl + "/ajax.php",
		    data : ({ 
		    ...
		    success: function(response)
		    {
		        ...
		        {
			       response = JSON.parse(response);
			       $("#content_to_update_"+cid).html(response.comment);
		        }
		    } 
	   	});
	...
	/*
	** This function is written to toggle all the thumbs for a video on hover
	*/
	function rotate_thumb(id, div_class, size) {
		if (typeof id !== undefined){
			if(!$('.thumbs-rotate-'+id).hasClass('hasthumbs')){
				$(div_class+id).addClass('hasthumbs');
				if (!size){
					size = 'original';
				}
				$.ajax({	
					url: baseurl+'/ajax/video_ajax.php',
					type: 'POST',
					dataType: 'json',
					data: { videoid: id, size : size},
					success: function(result)
					{
						var len = result.length;
						
				
						var _thumbarr = '<div class="showDiv"><div id="slideshow_circle" class="imgs_circles">';    
						for (var i = 0; i < len; i++) 
						{
							_thumbarr += ('<img class = "imgdiv" src = "'+ result[i] +'"></img>');
						}
						_thumbarr += "</div></div>";
						$(div_class+id).find('.video-link').append(_thumbarr);  
	...
	function get_video_details(video,selector){
		var videoid = video;
		...
		if (videoid){
			$.ajax({	
				...
				type: 'POST',
				dataType: 'json',
				data: { videoid: videoid, mode : 'video_details'},
				beforeSend : function(data){
					$(".thumb-holder").removeClass('tile-white-selected tile-white-is-selected');
				...
				{
					if (data.video){
						console.log('Data template e oye: '+data.template);
						console.log('Selector e oye: '+selector);
						$(selector).html(data.template);
	...
	function isEmptyCookie(str) {
    	return (!str || 0 === str.length);
	}


	// This function is used to copy the text of given element to clipboard
