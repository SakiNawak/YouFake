...
Ads.prototype.init = function (){
	...
	ads.events = [
  	...
  	switch (ads.settings.ad_type){
  		//DFP Ads
  		case '1':{
  			if (ads.player.options_.autoplay){
  		...
  		case '2':{
  			...
  			if (ads.settings.linear_type == 'pre-roll'){
				...
				if (ads.player.options_.autoplay){
					...
					var videoReadyState = setInterval(function(){
						if (ads.player.readyState() > 1){
							ads.initCbAdsPlayerContainer();
							ads.setCbAdsPlayerContents(ads.settings);
							clearInterval(videoReadyState);
						}
					},200);
			...
			//Conditioning for post-roll Ad	
  			...
  			//Conditioning for mid-roll Ad	
  				...
  				ads.player.on("timeupdate", ads.bind(ads, function(){
                    ...
                    perc = this.getPlayerCurrTimePerc(ads.player);
...
Ads.prototype.setCbAdsPlayerContents = function() {
    ...
    if (ads.adsplayer.device != 'mobile'){
...
Ads.prototype.onAdFullscreenClick_ = function() {
	...
	if (ads.isFunction(ads.adsplayer.adsManager.webkitRequestFullScreen)){
		if (ads.adsplayer.isFullscreen){
			...
			ads.adsplayer.adContainerDiv.webkitRequestFullScreen();	
		}
		...
		if (ads.adsplayer.isFullscreen){
			...
			ads.adsplayer.isFullscreen = true;
			ads.adsplayer.adContainerDiv.mozRequestFullScreen();	
		}
	}
...
Ads.prototype.destroyAdsPlayer_ = function(){
	...
	if (ads.adsplayer.CurrentRoundTime >= 5){
		...
		ads.adsplayer.adPlaying = false;
		ads.dikhaaoPlayer();
		if (ads.adsplayer.isFullscreen == true ){
			ads.adsplayer.isFullscreen = false;
			if (ads.isFunction(ads.adsplayer.adsManager.webkitRequestFullScreen)){
				document.webkitExitFullscreen();
			}else{
...
/*
* This method is used to add Ad markers in player seek bar 
*/
Ads.prototype.appendAdMarker = function() {
	var ads = this;
	videojs.registerComponent('AdMarker', videojs.extend(videojs.getComponent('Button'), {}));
	var controlBar = ads.player.getChild('controlBar');
	controlBar.getChild('progressControl').getChild('seekBar').addChild('AdMarker', {
		'el':videojs.createEl('div', 
			{ 	
				className: 'vjs-cb-ad-marker',
				dir: 'ltr',
				id : 'vjs-cb-ad-marker',
				innerHTML : '',
				style : 'padding:1.8px;height:5px !important;width:4px;background:yellow;margin-left:'+ads.settings.ad_time+'%;display:block !important;'
			},
			{ role : 'div'}
		)
	});
	...
	var currentTimePerc = 0;
	var ad_time = parseInt(ads.settings.ad_time);
	ads.player.on('timeupdate',function(){
		currentTimePerc = parseInt(ads.getPlayerCurrTimePerc(ads.player));
		if (currentTimePerc > ad_time && !childRemoved ){
			controlBar.getChild('progressControl').getChild('seekBar').removeChild('AdMarker')
			childRemoved = true;
		}
	});
}
...
/*
* This function is used to return and save player current time in percentage
*/
Ads.prototype.getPlayerCurrTimePerc = function(player){
	var duration = player.duration(),
	currentTime = player.currentTime(),
	playerPerc = (currentTime / duration * 100).toFixed();
	player.currentTimePerc = playerPerc;
	return parseInt(playerPerc);
}
