...
var cbVjsMigrate = function(player,options){
	...
	cbVjsMigrate.nextVideoLink = options.next_video_link;
	cbVjsMigrate.embed_check = options.embed_check;
	cbVjsMigrate.disable_embedded_videos = options.disable_embedded_videos;
	cbVjsMigrate.disable_embedded_msg = options.disable_embedded_msg;
	// console.log(options);
...
cbVjsMigrate.prototype.init = function(){
	...
	cbVjsMigrate.bindKeyShortCuts();
	cbVjsMigrate.saveVolume();
	
	if(cbVjsMigrate.embed_check==true){
		if(cbVjsMigrate.disable_embedded_videos=='yes'){
			cbVjsMigrate.player.createModal(disable_embedded_msg);
			// cbVjsMigrate.player.error(disable_embedded_msg);
		}
	}	
...
cbVjsMigrate.prototype.createCbVjsHeader = function(){
	...
	'</div>';
	if (!cbVjsMigrate.embed_check){
	cbVjsMigrate.cbVjsHeader.el_.innerHTML += '<div class="vjs-header-right-sec" data-toggle="modal" data-target="#share">'+
...
cbVjsMigrate.prototype.bindNextLink = function(){
	...
	cbVjsMigrate.player.on('ended',function(){
		if (cbVjsMigrate.nextPlaylistLink.length > 1){
			...
			var autoChk = getCookie('cb_autoPlayToggle');
			if(!isEmptyCookie(autoChk) && autoChk!='off'){
				if(cbVjsMigrate.nextVideoLink.length>1){
					window.location = cbVjsMigrate.nextVideoLink;
				}
			}
		}
	});
...
cbVjsMigrate.prototype.bindKeyShortCuts = function(){
	var cbVjsMigrate = this;
	// console.log(cbVjsMigrate);
	//cbVjsMigrate.player.el_.addEventListener("click", alert("yes"));
}
...
cbVjsMigrate.prototype.saveVolume = function(){
	var cbvjsvolume = this;
	cbvjsvolume.Currvol = "";
	cbvjsvolume.Muted = "";
	cbvjsvolume.vol_cookie = $.cookie("cb_volume");
	// console.log(cbvjsvolume.vol_cookie);
